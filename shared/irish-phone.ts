...
/** Longest matching dial code when `raw` is international; otherwise Ireland. */
export function matchPhoneCountry(raw: string): PhoneCountry {
  const trimmed = raw.trim()
  if (!trimmed.startsWith('+') && !trimmed.startsWith('00')) {
    return getPhoneCountry('IE')
  }
  const digits = trimmed.replace(/\D/g, '')
  const international = trimmed.startsWith('00') ? digits.slice(2) : digits
  let best = getPhoneCountry('IE')
  let bestLen = 0
  for (const country of PHONE_COUNTRIES) {
    const code = country.dial.replace(/\D/g, '')
    if (international.startsWith(code) && code.length > bestLen) {
      best = country
      bestLen = code.length
    }
  }
